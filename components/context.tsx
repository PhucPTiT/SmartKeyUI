...
const Context = ({onDelete, onUpdate, x, y, closeContextMenu, item} : ContextProps) => {
    ...
    return ( 
        <div 
            ...
            <div
                onClick={() => 'id' in item && onUpdate && onUpdate(item)} 
                className="px-2 py-1 flex w-40 justify-between hover:bg-primary-foreground/10 cursor-pointer rounded-sm">
                <span>Update</span>
                <Pencil/>
            </div>
        </div>
