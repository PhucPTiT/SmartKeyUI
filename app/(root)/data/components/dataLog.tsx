...
const DataLog = () => {
    ...
    const [rowEdit, setRowEdit] = useState<DataItem | {}>({})
    ...
    }, [page, reload, date, sortColumn, sortDirection, rowDelete, rowEdit, key])
    ...
    //Feature Edit
    const handlePopupEdit = (dataSensor: DataItem | {}) => {
      setRowEdit(dataSensor);
    }

    return (
      <div className="h-full" > 
          {'id' in rowDelete && <PopupDelete data = {rowDelete} onHandle={handlePopupDelete}/>}
          {'id' in rowEdit && <PopupUpdate data = {rowEdit} onHandle={handlePopupEdit}/>}
          {contextMenu.show && <Context onDelete={handlePopupDelete} onUpdate={handlePopupEdit} item={contextMenu.item} x = {contextMenu.x} y = {contextMenu.y} closeContextMenu={contextMenuClose}/>}
          <div>
